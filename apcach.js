...
function composeColor(contrast, chroma, hue, alpha = 100, fgIsBlack = true) {
  let lightness = composeLightess(parseFloat(contrast), parseFloat(chroma), parseFloat(hue), fgIsBlack);
  return { contrast: contrast, lightness: lightness, chroma:chroma, hue: hue, alpha: alpha, fgIsBlack: fgIsBlack};
}
...
function composeLightess(targetContrast, chroma, hue, fgIsBlack) {
  ...
  var factContrast = 0;
  var deltaContrast = 0
  var lightness = fgIsBlack ? targetContrast / 100 : (100 - targetContrast) / 100;
  var lightnessPatch = 0.5;
  var iteration = 0;
  while (Math.abs(factContrast - targetContrast) > 0.01 && iteration < 20) {
    iteration++;
    let oldLightness = lightness;
    var bgColor = formatCss({ mode: 'oklch', l: oldLightness, c: chroma, h: hue })
...
function inP3(color) {
  ...
  return inGamutP3(formatOKLCH(color));
}
...
export { composeColor, formatOKLCH, APCAcontrast, inP3 }